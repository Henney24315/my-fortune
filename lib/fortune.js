...
function profileFromCookieHeader(cookieHeader) {
  const cookies = {};
  ...
  if (!cookies.fortune_profile) return null;

  try {
    return JSON.parse(decodeURIComponent(cookies.fortune_profile));
  } catch {
    return null;
  }
}

module.exports = { fetchAll, profileFromCookieHeader };
