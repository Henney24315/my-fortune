...
function describeScore(scoreText) {
  return SCORE_LABELS[scoreText] || scoreText;
}
...
async function fetchMonthly(profile, year, month) {
  ...
  firstCont.find('.day_unse_table .li_01').each((_, li) => {
    ...
    const specialDay = $(cells[1]).text().trim();
    const content = $(cells[2]).text().replace(/\s+/g, ' ').trim();
    const score = $(cells[3]).find('.text').text().trim();
    if (dayNum) {
      days.push({ day: dayNum, content, score, scoreLabel: describeScore(score), specialDay });
    }
  });
...
async function fetchAll(profile) {
  ...
  const [todayHighlight, tomorrowHighlight] = await Promise.all([
  ...
  for (let i = 0; i < 7; i += 1) {
    ...
    const monthData = await getMonth(date.year, date.month);
  ...
  if (next7Days[0]) {
    todayHighlight.score = next7Days[0].score;
    todayHighlight.scoreLabel = next7Days[0].scoreLabel;
    todayHighlight.specialDay = next7Days[0].specialDay;
  }
  if (next7Days[1]) {
    tomorrowHighlight.score = next7Days[1].score;
    tomorrowHighlight.scoreLabel = next7Days[1].scoreLabel;
    tomorrowHighlight.specialDay = next7Days[1].specialDay;
  }
